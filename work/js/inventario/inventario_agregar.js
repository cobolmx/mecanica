$(document).ready(function () {
    ...
    $('#inventarioCategoria').on('change', function () {
        $('#inventarioMarca').find('option').remove().end().append('<option value="">Selecione una marca</option>');
        $.get("work/php/inventario/obtener/obtener_marca_chained.php", { id_categoria: this.value }, function (data) {
            $('#inventarioMarca').append(data);
        });
    });
    $('#inventarioMarca').on('change', function () {
        $('#inventarioModelo').find('option').remove().end().append('<option value="">Selecione una modelo</option>');
        $.get("work/php/inventario/obtener/obtener_modelo_chained.php", { id_marca: this.value }, function (data) {
    ...
    $("#inventarioForma").validate({
        ...
        messages: {
            ...
            inventarioCantidad: "La cantidad debe de ser mayo a 1."
        },
